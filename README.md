...
`--dsl` 是唯一的代码风格分类；`--devices`（本地）或 `--worker-url`（远端）
必选其一作为硬件目标。backend / arch / device_type 全部由这两者自动派生 —
**用户不再手写 `--backend` / `--arch`**。完整 DSL 列表和派生规则见
[DSL 分派层](#dsl-分派层)。
...
- `backend` ← DSL 名字（`triton_ascend` → `ascend`；`cuda_c` → `cuda`；`cpp` → `cpu` …；硬编码表见 [hw_detect.DSL_BACKEND](.autoresearch/scripts/hw_detect.py)）
- `arch`：
...
由 autoresearch 自带，把项目（或至少 `.autoresearch/scripts/`）rsync 到
worker 机器即可启动。
...
客户端只用 stdlib + pyyaml 通信，上述依赖仅 worker 侧装。tarball 内带
`ar_vendored/`，worker 解包即可 `import ar_vendored`。
